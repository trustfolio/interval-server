...
If the user's command included `--directed`, persist it for the build steps; otherwise clear any stale flag:

```bash
rm -f graphify-out/.graphify_directed
# touch graphify-out/.graphify_directed   # run this line only when --directed was given
```
...
**Before starting:** if `--directed` was given, ensure `graphify-out/.graphify_directed` exists (see Step 1). The script below reads that marker and passes `directed=True` to `build_from_json()`, producing a `DiGraph` that preserves edge direction (source→target) instead of the default undirected `Graph`.
...
directed = Path('graphify-out/.graphify_directed').exists()
